    ...
    const response = await portal("/chat/completions", {
      ...
      payload: {
        ...
        messages: [
          {
            role: "system",
            content: "Ты анализируешь статус и суммируешь только актуальные оценки времени. Прими только строки из сообщения пользователя. Сначала смыслово прочитай весь текст: не складывай автоматически все числа. Полностью игнорируй любую оценку, перед которой в той же фразе стоят слова «итого», «суммарно» или «всего» — это справочная запись, не самостоятельная оценка. Например, из «до 10 ч., до 2 ч. (доп. условия, итого до 12 ч.)» учитывай только 10 и 2, результат 0-12. Исключай оценку, если по смыслу она отклонена, отменена, не выбрана, не согласована, устарела или заменена другой; это относится к любым формулировкам, не только к точным словам «отказались» или «не согласовано». Варианты с пометками «вар 1», «вар. 1», «вариант 1» и подобными являются взаимоисключающими. Если текст сообщает, что выбран вариант N, учитывай только его; остальные варианты исключай. Если варианты есть, но выбор неясен, не учитывай их. Например, из «10-12 ч. (отказались), 8-10 ч.» нужно взять только 8-10. Только после этого сложи независимые оставшиеся оценки: интервал A-B складывай как A с A и B с B; «до X» и фиксированное X считай как 0-X. Нечитаемые строки игнорируй. Ответь строго одним интервалом min-max только цифрами и дефисом, без пояснений, единиц и Markdown. Если нет ни одной актуальной читаемой оценки, ответь пустой строкой.",
          },
